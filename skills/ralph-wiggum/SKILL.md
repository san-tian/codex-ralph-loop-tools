...
- Before concluding the host is healthy but tools are missing, run `scripts/install_codex_plugin.py --check`; if it reports stale plugin install/cache state, refresh with `scripts/install_codex_plugin.py` and restart or reload Codex
...
- The supported no-source deployment refreshes plugin discovery/cache with `scripts/install_codex_plugin.py` and installs lifecycle hooks through `/vePFS-Mindverse/user/intern/ccss/.codex/hooks.json`, generated by `scripts/install_workspace_hooks.py`
