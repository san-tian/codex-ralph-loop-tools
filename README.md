...
First check whether the installed cache `.mcp.json` contains absolute paths. The source `.mcp.json` is intentionally relative for portability, but the installed mirror/cache should be generated by `scripts/install_codex_plugin.py` with an absolute `cwd` and absolute `scripts/ralph_loop_mcp_server.py` argument.
